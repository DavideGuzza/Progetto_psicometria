...
electrode_coords <- data.frame(
  ...
  # Scale down coordinates by factor of 0.5-0.6 to match head size
  x = c(-0.23, -0.50, -0.71, -0.50, -0.71, -0.87, -0.50, -0.71, -0.38, -0.23, 
        -0.13, 0.00, 0.00, 0.00, 0.23, 0.00, 0.50, 0.71, 0.50, 0.00, 
        0.00, 0.71, 0.87, 0.50, 0.71, 0.38, 0.23, 0.13) * 85,
  y = c(0.91, 0.85, 0.71, 0.45, 0.00, -0.23, -0.45, -0.71, -0.92, -0.91, 
        -0.98, -1.00, -0.50, 0.00, 0.91, 0.85, 0.85, 0.71, 0.45, 0.53, 
        0.00, 0.00, -0.23, -0.45, -0.71, -0.92, -0.91, -0.98) * 85
)
# Plot with labels
ggplot(electrode_coords, aes(x = x, y = y)) +
  # Plot points
  geom_point(size = 3, color = "red") +  # Customize point appearance
  # Add labels (adjust `vjust` to position text above/below points)
  geom_text(aes(label = electrode), 
            size = 3, 
            vjust = -1.2,  # Moves text upward; use +1.2 to move downward
            check_overlap = TRUE) +  # Avoids overlapping labels
  # Add head outline (if using eegUtils) # Requires `eegUtils`
  # Adjust plot appearance
  geom_head(linewidth = 1.5, colour = "black") +  
  coord_fixed() +  # Ensures proper aspect ratio
  theme_void()     # Removes axes/background
```


```{r}
df_CAD <- df %>% 
  dplyr::filter(time >= 150, time <= 190,channel %in% c("F4","FC4","F8")) 
...
```{r}
data(eegcoord)
eegcoord$channel <- rownames(eegcoord)
df <- df %>%
  mutate(channel=toupper(channel)) %>% 
  left_join(eegcoord[,c(4,5,6)], by = c("channel" = "channel")) %>% 
  mutate(Lateral=case_when(
    xproj < -0.05 ~ "Left",
    xproj > 0.05 ~ "Right",
    TRUE ~ "Middle"
  ))
dataCoeff=obs2coeffWithin(value~ channel*Lateral*condition,data=df,units=~id)
```
